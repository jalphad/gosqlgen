...
func (db *DB) Comments() builder.KnownTableStartQuery[CommentsDto, *CommentsDto] {
	return NewCommentsQuery(db.pool)
}

// PostTags returns a query builder for post_tags
func (db *DB) PostTags() builder.KnownTableStartQuery[PostTagsDto, *PostTagsDto] {
	return NewPostTagsQuery(db.pool)
}

// Posts returns a query builder for posts
func (db *DB) Posts() builder.KnownTableStartQuery[PostsDto, *PostsDto] {
	return NewPostsQuery(db.pool)
}

// Tags returns a query builder for tags
func (db *DB) Tags() builder.KnownTableStartQuery[TagsDto, *TagsDto] {
	return NewTagsQuery(db.pool)
}

// Users returns a query builder for users
func (db *DB) Users() builder.KnownTableStartQuery[UsersDto, *UsersDto] {
...
//func (tx *Tx) Comments() *builder.KnownTableBuilder[CommentsDto, *CommentsDto] {
//	q := NewCommentsQuery(nil)
//	return q.WithTx(tx.tx)
//}
//
//// PostTags returns a query builder within the transaction
//func (tx *Tx) PostTags() *builder.KnownTableBuilder[PostTagsDto, *PostTagsDto] {
//	q := NewPostTagsQuery(nil)
//	return q.WithTx(tx.tx)
//}
//
//// Posts returns a query builder within the transaction
//func (tx *Tx) Posts() *builder.KnownTableBuilder[PostsDto, *PostsDto] {
//	q := NewPostsQuery(nil)
//	return q.WithTx(tx.tx)
//}
//
//// Tags returns a query builder within the transaction
//func (tx *Tx) Tags() *builder.KnownTableBuilder[TagsDto, *TagsDto] {
//	q := NewTagsQuery(nil)
//	return q.WithTx(tx.tx)
//}
...
func NewCommentsQuery(pool *pgxpool.Pool) builder.KnownTableStartQuery[CommentsDto, *CommentsDto] {
	return builder.NewKnownTableBuilder(&CommentsDto{}, pool)
}

// NewPostTagsQuery returns a query builder for post_tags
func NewPostTagsQuery(pool *pgxpool.Pool) builder.KnownTableStartQuery[PostTagsDto, *PostTagsDto] {
	return builder.NewKnownTableBuilder(&PostTagsDto{}, pool)
}

// NewPostsQuery returns a query builder for posts
func NewPostsQuery(pool *pgxpool.Pool) builder.KnownTableStartQuery[PostsDto, *PostsDto] {
	return builder.NewKnownTableBuilder(&PostsDto{}, pool)
}

// NewTagsQuery returns a query builder for tags
func NewTagsQuery(pool *pgxpool.Pool) builder.KnownTableStartQuery[TagsDto, *TagsDto] {
	return builder.NewKnownTableBuilder(&TagsDto{}, pool)
}

// NewUsersQuery returns a query builder for users
func NewUsersQuery(pool *pgxpool.Pool) builder.KnownTableStartQuery[UsersDto, *UsersDto] {
